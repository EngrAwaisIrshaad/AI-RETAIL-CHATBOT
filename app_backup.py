...
# MongoDB Setup
MONGO_URI = "mongodb+srv://engrawaisawan358:cczM9vQqeKtK9u0i@cluster0.zc395b4.mongodb.net/?appName=Cluster0"
try:
...
def chat():
    ...
    # Debug: Print current stage and user message
    print(f"DEBUG: Stage={session.get('stage', 'None')}, User={user_msg}")

    # 1. Explicit Restart Only
    ...
    # 2. Handle Goodbyes/Thanks at any time
    ...
    # A. Product Selection (IMPROVED - checks for any alias in the message)
    if stage == "product":
        for product_name, product_info in PRODUCTS.items():
            if any(alias in user_msg for alias in product_info["aliases"]):
                ...
                return jsonify(
                    {"msg": f"Great! You selected {product_name}. Types available: {', '.join(product_info['types'])}"})
    ...
    # C. Quantity Selection
        ...
        if user_msg.isdigit():
            ...
            session.modified = True
            return jsonify(
                {"msg": "Items added to cart! Please enter your payment method (paypal, debit card, credit card)."})
        return jsonify({"msg": "Please enter a valid number (1, 2, 3, etc.)."})

    # D. Payment Processing (FIXED - now processes immediately)
    elif stage == "payment":
        # Get order details
        product = session.get("product")
